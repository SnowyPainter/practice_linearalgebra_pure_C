...
void viz_arrow(Viz* v, float ox,float oy, float vx,float vy, float scale, float cx,float cy, RGB color, const char* label);
void viz_angle(Viz* v, float ax, float ay, float bx, float by, float radius, float scale, float cx,float cy, RGB color, char* out_deg_text, size_t out_len);
